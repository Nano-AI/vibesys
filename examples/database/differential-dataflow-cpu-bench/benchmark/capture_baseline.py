...
target**: the vanilla vendored differential-dataflow source in `_ref_engine/`,
...
the vendored `_ref_engine/` source or the host changes.
...
def main():
    ...
    if not os.path.isdir(_REF_ENGINE):
        print(f"ERROR: vendored _ref_engine not found at {_REF_ENGINE}", file=sys.stderr)
        return 2
