...
- **Output-equivalence is absolute.** `accuracy_checker/equivalence_gate.py`
...
`benchmark/benchmark.py` reports the headline
...
is round 0 itself, captured once on this box in `benchmark/baseline.json` by
`benchmark/capture_baseline.py`. CPU is the child process's exact
