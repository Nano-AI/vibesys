...
from vibe_database.constants import _BOLD, _CYAN, _DIM, _GREEN, _RED, _RESET, _YELLOW
...
class TestFormatTokenCount:
    def test_zero(self):
        from vibe_database.agents.callbacks import _format_token_count
...
class TestDefaultContextWindowLookup:
    def test_claude_4_6_resolves_to_1m(self):
        from vibe_database.agents.callbacks import _default_context_window_lookup
...
class TestPrefixFormat:
    ...
    def test_elapsed_time_advances(self, capsys, monkeypatch):
        ...
        from vibe_database.agents import callbacks
...
class TestUpdateUsagePublicHook:
    ...
    ``AgentLogger.on_usage`` routes per-turn usage dicts from ``vibe_database._agent_cli``
