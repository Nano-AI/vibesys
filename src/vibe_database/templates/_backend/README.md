...
`Prompt(template_dir, backend)` (in `vibe_database/prompts.py`) auto-injects every fragment under `_backend/<backend>/` as a kwarg keyed by **filename stem** on every `prompt.render(...)` call.
...
1. Add the variant to `ComputeBackend` in
   `vibe_database/constants.py`.
2. Create `vibe_database/templates/_backend/<new>/` and mirror every
...
The canonical list of fragment names lives on `ComputeBackendFragment.NAMES` (in `vibe_database/prompts.py`), not in this directory layout. Adding a fragment is a 3-step change:
