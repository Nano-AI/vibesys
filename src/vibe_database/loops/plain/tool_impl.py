...
Both the LangChain ``@tool`` callables in :mod:`vibe_database.loops.plain.tools`
and the FastMCP ``@mcp.tool()`` callables in
:mod:`vibe_database.loops.plain.mcp_server` delegate into the helpers in this
...
:mod:`vibe_database.loops.plain.issue_board` (which itself only imports ``pydantic``).
