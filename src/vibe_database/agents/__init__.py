...
runner classes are imported, so the public API of this package is::
    ...
    from vibe_database.agents import AgentRunner, build_agent_runner
"""
...
from vibe_database.constants import DEFAULT_AGENT_BACKEND

from .base import AgentRunner

# ``CliAgentRunner`` and ``DeepAgentsRunner`` are imported lazily to break a
# circular import: ``vibe_database.agent_runner`` imports
...
def build_agent_runner(
    ...
    *,
    ...
    Args:
        config: Parsed :class:`~vibe_database.config.Config`; the ``[agent]``
        ...
        agent_backend: CLI override; if set, takes precedence over
            ...
            :data:`vibe_database.constants.DEFAULT_AGENT_BACKEND` (``"cli"``).
        ...
        skill_source_dirs: Absolute source paths of skill directories — the
            ...
            on each invoke.
        model: Result of :func:`vibe_database.llm_client._build_model` — only
